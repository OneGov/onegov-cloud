...
class Scenario(object):
    ...
    def fixture(self, name):
        return self.pytest_request.getfixturevalue(name)
    ...
    def c(self):
        ...
        return Collections(self.session)

    @cached_property
    def session(self):
        # returns the session used by the currently active feriennet app
        for name in self.pytest_request.fixturenames:
            if name in ('feriennet_app', 'es_feriennet_app'):
                return self.fixture(name).session()
    ...
    def add_user(self, complete_profile=True, **columns):
        ...
        self.users.append(self.add(
            model=User,
            password_hash=self.fixture('test_password'),
            **columns
        ))
    ...
    def add_activity(self, **columns):
        ...
        self.activities.append(self.add(VacationActivity, **columns))
