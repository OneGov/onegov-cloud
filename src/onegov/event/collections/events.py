...
class EventCollection(Pagination):
    ...
    def as_xml(self):
        """
        Returns the events of the given occurrences as xml.
        ...
        # for ev in EventCollection(self.session).query().all():
        # for e in self.session.query(Event).all():
        for e in self.query().all():
