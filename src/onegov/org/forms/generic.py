...
class ChangeUrlForm(Form):
    ...
    def is_duplicate_entry(self, model: Any, name: str) -> bool:
        cls = get_polymorphic_base(model.__class__)
    ...
    def validate_name(self, field: StringField) -> None:
        ...
        if self.is_duplicate_entry(model, normalized_name):
            raise ValidationError(
                _('An entry with the same name exists')
            )


class ChangeAdjacencyListUrlForm(ChangeUrlForm):

    def is_duplicate_entry(self, model: AdjacencyList, name: str) -> bool:
        cls: type[AdjacencyList] = get_polymorphic_base(model.__class__)  # type: ignore[assignment]
        session = self.request.session
        query = session.query(cls).filter(
            cls.parent_id == model.parent_id,
            cls.name == name,
            cls.id != model.id,
        )
        return session.query(query.exists()).scalar()
