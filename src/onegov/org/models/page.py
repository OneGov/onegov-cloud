...
def migrate_page_links(session, old_domain, new_domain, test=False):

    fields = ('lead', 'text', 'url')
    found = 0
    pattern = re.compile(r'(https?://)' + f'({re.escape(old_domain)})')

    def repl(matchobj):
        return f'{matchobj.group(1)}{new_domain}'

    def migrate(page, test=False):
        nonlocal found
        for field in fields:
    ...
    for item in session.query(Topic):
        migrate(item, test)
