...
@cli.command(name='indesign-to-html')
@click.argument('zip_file', type=click.Path(exists=True, resolve_path=True))
def indesign_to_html(
    zip_file: str,
) -> Callable[[OrgRequest, OrgApp], None]:
    """ Combines and converts InDesign exported HTML files to clean HTML.

    Example:
        onegov-org indesign-to-html path/to/html_folder
    """

    def combine_and_clean_html(request: OrgRequest, app: OrgApp) -> None:
        import os
        from pathlib import Path
        import re
        from bs4 import BeautifulSoup
        ...
        if file.suffix == '.zip':
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                if not html_dir:
                    click.secho(f"No 'html' directory found in {zip_file}", fg="red")
                    return
                
                path = Path(html_dir)
                
                # Move all this processing INSIDE the context manager
                html_files = sorted(
                    [f for f in path.glob("*.html") if f.name != "combined_clean.html"],
                    key=lambda f: [int(s) if s.isdigit() else -1 for s in re.findall(r'\d+|\D+', f.name)]
                )
                
                if not html_files:
                    click.secho(f"No HTML files found in {html_dir}", fg="red")
                    return
                    
                click.echo(f'Found {len(html_files)} HTML files in {html_dir}')
                
        
                # Create new HTML document for the combined content
                ...
                for file_path in html_files:
                    ...
                    for textframe in textframes[1:] if textframes else []:
                        click.secho(f"Textframe", fg="cyan")
                    ...
                    while i < len(all_paragraphs):
                        ...
                        # Check if paragraph has div without class as parent
                            ...
                            if '_01-Titel' in p_class:
                                ...
                                tag_type = 'h2'
                            elif '_003-Titel' in p_class or 'Titel--1-2' in p_class:
                                tag_type = 'h3'
                            elif 'Vorlage_Vorlage-Titel' in p_class:
                                tag_type = 'h2'
                            elif 'table' == p_class:
                                if i>0 and all_paragraphs[i-1]['class'] != 'table':
                                    click.secho(f"Table found", fg="yellow")
                ...
                # output_file = os.path.join(html_dir, 'combined_clean.html')

                                # DO ALL YOUR HTML PROCESSING HERE
                # Create combined_html, process files, etc.
                
                # Save the output file inside the temp directory first, then copy it out
                temp_output = os.path.join(html_dir, 'combined_clean.html')
                final_output = os.path.join(os.path.dirname(zip_file), 'combined_clean.html')
                
                # ... your HTML processing code ...
                
                # Copy the result out of the temp directory
                # import shutil
                # shutil.copy2(temp_output, final_output)
                
                click.secho(f"Combined and cleaned HTML saved to {final_output}", fg="green")
                
                # # Write the cleaned HTML to the output file
                with open(final_output, 'w', encoding='utf-8') as f:
                    f.write(combined_html.prettify())
                
                click.echo(f"Processed {len(html_files)} HTML files")

    return combine_and_clean_html