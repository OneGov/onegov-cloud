...
class ReturnToMixin:
    ...
    *Do not use the return-to parameter directly*. Redirect parameters
    ...
    if TYPE_CHECKING:
        ...
        def url(self) -> str: ...
        @property
        def GET(self) -> GetDict: ...  # ruff: ignore[invalid-function-name]
...
class CoreRequest(
    ...
    def x_vhm_host(self) -> str:
        """ The X_VHM_HOST variable or an empty string.
    ...
    def x_vhm_root(self) -> str:
        """ The X_VHM_ROOT variable or an empty string.
    ...
    def path_url(self) -> str:
        """ The path_url, taking virtual hosting into account. """
    ...
    def theme_link(self) -> str:
        """ The link to the current theme. Computed once per request.
    ...
    def browser_session(self) -> BrowserSession:
        """ A browser_session bound to the request. Works via cookies, so
        requests without cookies won't be able to use the browser_session.
    ...
    def translator(self) -> Callable[[str], str]:
        """ The translate function for basic string translations. """
    ...
    def is_logged_in(self) -> bool:
        """ True if the current request is logged in at all. """
        return self.identity is not NO_IDENTITY

    @cached_property
    def agent(self) -> ua_parser.DefaultedResult:
        """ The user agent, parsed by ua-parser. """
    ...
    def current_role(self) -> str | None:
        """ The user-role of the current request, or None if not logged in. """
    ...
    def analytics_code(self) -> Markup | None:
        """ The embeddable code for the active analytics provider. """
