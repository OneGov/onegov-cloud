...
stripe.default_http_client = stripe.RequestsClient(
    session=niquests.Session(timeout=80)  # type: ignore[arg-type]
)
...
class StripePayment(Payment):
    ...
    def charge(self) -> stripe.Charge:
        with stripe_api_key(self.provider.access_token):
            assert self.remote_id is not None
...
class StripeConnect(PaymentProvider[StripePayment]):
    ...
    latest_payout: dict_property[str | None] = meta_property()
    ...
    def public_identity(self) -> str:
        ...
        # NOTE: This first getattr probably doesn't work anymore
    ...
    def oauth_url(
        ...
        redirect_uri: str,
        state: str,
        user_fields: stripe.OAuth.OAuthAuthorizeUrlParamsStripeUser
    ) -> str:
        """ Generates an oauth url to be shown in the browser. """

        return stripe.OAuth.authorize_url(
            # NOTE: the method will raise a nicer error than an assert
            #       would raise for us.
    ...
    def prepare_oauth_request(
        ...
        return self.oauth_url(
            redirect_uri=f'{self.oauth_gateway}/redirect',
            state=response.json()['token'],
            # FIXME: We could try to validate user_fields or at least
            #        only inlcude the data Stripe expects, we could
            #        accomplish this by introspecting the TypedDict
            user_fields=user_fields or {}  # type: ignore[arg-type]
        )
    ...
    def sync_payment_states(self, session: Session) -> None:
        ...
        def payments(ids: Collection[str]) -> Query[StripePayment]:
