...
def test_views_scan_job(client):
    ...
    with freeze_time("2019-01-01"):
        ...
        add.form['dispatch_date'].select("2019-01-05")
    ...
    with freeze_time("2019-01-02"):
        ...
        edit.form['type'].select("express")
        edit.form['dispatch_date_express'] = "2019-01-03"
    ...
    view = client.get('/scan-jobs').click("03.01.2019")
    assert "Scan-Aufrag 1: My Municipality, 03.01.2019" in view
    assert all([f">{number}<" in view for number in range(1, 16 + 1)])
    assert "Lieferung an das Steueramt Winterthur am 03.01.2019" in view
    ...
    client.logout()
    client.login_admin()
    deleted = client.get('/scan-jobs').click("03.01.2019").click("Löschen")
    assert deleted.status_int == 200
    assert "03.01.2019" not in client.get('/scan-jobs')
...
def test_views_scan_jobs_permissions(mock_method, client):
    ...
    with freeze_time("2019-01-01"):
        ...
        add.form['dispatch_date_express'] = "2019-01-05"
