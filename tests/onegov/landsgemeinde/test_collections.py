...
def test_assembly_collections(session):
    assemblies = AssemblyCollection(session)
    assert assemblies.query().count() == 0
    assert assemblies.by_id(None) is None
    assert assemblies.by_date(None) is None
    ...
    assert assemblies.by_date(date(2023, 5, 7)).date.year == 2023
    assert assemblies.by_date(date(2021, 5, 7)).date.year == 2021

    assert assemblies.by_id(assembly.id).date.year == 2022


def test_agenda_item_collections(session):
    assemblies = AssemblyCollection(session)
    ...
    assert items_1.by_number(1).date.year == 2021
...
def test_votum_collection(session):
    ...
    assert vota.by_id(None) is None
    assert vota.by_number(None) is None
