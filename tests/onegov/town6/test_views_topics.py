...
def test_topic_keywords(client: Client) -> None:
    ...
    assert [
        keyword.text for keyword in page.pyquery('.page-keywords .blank-label')
    ] == ['Einwohnerkontrolle', 'Einwohneramt']
    assert page.pyquery('.page-keywords strong').text() == 'Schlagworte'

    editor = client.spawn()
    editor.login_editor()
    editor_page = editor.get('/topics/themen/einwohnerdienste')
    assert editor_page.pyquery('.page-keywords')
