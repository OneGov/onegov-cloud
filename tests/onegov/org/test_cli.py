...
def _ogi_create_instance(cfg_path: str) -> None:
...
def _ogi_run_import(cfg_path: str, file: str, *extra: str) -> Any:
...
def test_import_from_ogi_scraper(
    ...
    cfg_path = _ogi_write_cfg(postgres_dsn, temporary_directory, redis_url)
    _ogi_create_instance(cfg_path)
    file = _ogi_write_export(temporary_directory)

    result = _ogi_run_import(cfg_path, file)
    assert result.exit_code == 0
...
def test_import_dry_run(
    ...
    result = _ogi_run_import(cfg_path, file, '--dry-run')
    assert result.exit_code == 0
...
def test_import_warns_on_missing_image(
    ...
    cfg_path = _ogi_write_cfg(postgres_dsn, temporary_directory, redis_url)
    _ogi_create_instance(cfg_path)
    file = _ogi_write_export(temporary_directory, entries=[
