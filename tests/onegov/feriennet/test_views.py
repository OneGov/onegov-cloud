...
@pytest.mark.parametrize('scenario_name,period_config,user_config,activity_config,booking_config,invoice_config,expected_deleted,expected_message', [
    # Scenario 1: Delete user with past bookings and paid invoices (should succeed)
    (
        'past_paid',
        {'title': '2020', 'active': False},
        {'username': 'test_user@example.org', 'role': 'member', 'complete_profile': True},
    ...
    # Scenario 2: Try to delete user with unpaid invoices from finalized period (should fail)
    (
        'unpaid_finalized',
        {'title': '2021', 'active': True, 'confirmed': True},
        {'username': 'unpaid_user@example.org', 'role': 'member', 'complete_profile': True},
    ...
    # Scenario 3: Delete user with unpaid invoice from unfinalized period (should succeed)
    (
        'unpaid_unfinalized',
        {'title': '2022', 'active': True, 'finalized': False},
        {'username': 'unfinalized_user@example.org', 'role': 'member', 'complete_profile': True},
    ...
    # Scenario 4: Try to delete user with bookings in active period (should fail)
    (
        'active_booking',
        {'title': '2023', 'active': True},
        {'username': 'active_user@example.org', 'role': 'member', 'complete_profile': True},
    ...
    # Scenario 5: Try to delete organizer with activities (should fail)
    (
        'organizer_with_activities',
        {'title': '2024', 'active': True},
        {'username': 'organizer@example.org', 'role': 'editor', 'complete_profile': True},
        {'title': 'Coding Class', 'state': 'accepted', 'username': 'organizer@example.org'},
        None,  # No booking needed
        None,  # No invoice
        False,
...
def test_delete_user_scenarios_parametrized(
    ...
    # Setup period
    scenario.add_period(**period_config)
    ...
    if invoice_config is not None:
        ...
        if invoice_config.get('paid') is not None and invoice.items:
            for item in invoice.items:
                item.paid = invoice_config['paid']
            session.flush()
        
        # Confirm period for unfinalized period
        if period_config.get('finalized') is False and period_config.get('active'):
            page = admin.get('/matching')
            page.form['confirm'] = 'yes'
            page.form['sure'] = 'yes'
            page.form.submit()
...
def test_admin_user_no_delete_button(client: Client, scenario: Scenario) -> None:
